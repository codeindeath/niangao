...
CN2EN = {
    ...
    "饮食": "diet", "运动": "exercise", "宠物": "pets", "旅行": "travel",
    "衣着": "fashion", "养护": "selfcare", "购物": "shopping", "娱乐": "fun",
        ...
        try:
            resp = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"{USER_PREFIX}{content}"},
                ],
                temperature=0.1,
                max_tokens=60,
                timeout=15,
...
for i, (eid, content) in enumerate(rows):
    try:
        ...
        if err > 5:
            break
    if (i + 1) % 10 == 0:
        print(f"  {i+1}/{len(rows)} ok={ok} err={err}")
