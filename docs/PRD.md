...
**交互模式**：类似抖音的纵向全屏滑动，每张卡片占满几乎整个屏幕。

| 功能点 | 详细说明 |
|--------|---------|
| 数据源 | 登录 → `/recommend?limit=20&offset=N`；未登录 → `/experiences?page=N&page_size=20&sort=latest` |
| 顶部三标签 | **推荐** \| **我的** \| **收藏**，均分宽度，透明背景，选中态粗体黑字 + 绿色短下划线（3px）。方案A 抖音经典顶栏 |
| 标签数据切换 | 推荐 → `fetchRecommendations`；我的 → `fetchMyExperiences`；收藏 → `fetchMyBookmarks` |
| 左右滑切标签 | 容器的 Touch 事件监听（`onTouchStart`/`onTouchEnd`），循环式：左滑 → 下一标签，右滑 → 上一标签，首尾相连 |
| 全屏卡片 | 卡片高度 = 屏幕高度 − 安全区域 − 顶部 tab 栏(44px) − 底部 tab 栏(80px) − 边距(8px) |
| 3D 卡片翻转 | 点击卡片触发 `Animated.spring` Y 轴 3D 翻转；正面 = 经验内容，背面 = AI 解读（如有） |
| 卡片内容区 | 顶部偏移避开透明 tab 栏覆盖区（`contentTop = insets.top + 44 + 12`），领域标签和翻转提示均推算至 tab 栏下方 |
| 无限滚动 | `onEndReached` 触发自动加载下一页，offset 递增 20，底部 loading 指示器 |
| 到底提示 | 数据全部加载完毕显示「— 已经到底了 —」 |
| 空状态 | 推荐：「暂无推荐内容」+「发布经验后，推荐会更精准」；我的：「你还没有发布经验」+「分享一条你的经验吧」；收藏：「你还没有收藏经验」+「去发现页面收藏喜欢的经验」 |
| 错误态 | 「加载失败，请检查网络连接」+ 重试按钮 |
| 下拉刷新 | RefreshControl，重新请求（offset 重置为 0） |
| 点赞 | 乐观更新 UI（即点即变），API 失败则回滚 |
| 收藏 | 同上；已收藏显示「★ 已收藏」（金色 `#e8a850`） |
| 卡片信息 | 头像(首字母) + 创作者名称 + 称号 + 领域标签 + 内容 + 价值度星级 + 互动按钮 |
| 创作者 | 优先 `creator_name`，其次 `author_name`，兜底「匿名」 |
| 称号 | 用户自定义 ≤20 字符（rune 计数），个人主页编辑，卡片和主页均展示 |
| 领域标签 | 优先显示二级领域名（如「时间管理」），未知则回退一级（如「生活智慧」） |
| 平台标签 | `source_type='platform'` 时显示绿色圆形「官」标签 |
| 翻转提示 | 卡片右上角「点击翻转 ↻」（仅当有解读时显示） |
...
|--------|---------|
| 头像 | 圆形，首字母，深绿底色 |
| 自定义称号 | 可编辑的称号（≤20 字符，rune 计数），卡片和主页均展示。昵称下方点击「修改称号」弹出输入框 |
...
| title | VARCHAR(20) | 自定义称号，≤20 字符（rune 计数） |
...
- 管理后台访问：http://115.190.177.146/admin（账号和密码从生产环境安全配置获取，不写入仓库文档）
